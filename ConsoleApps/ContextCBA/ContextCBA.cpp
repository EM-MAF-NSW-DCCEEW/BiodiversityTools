...
int main(int argc, char *argv[])
{
	...
	if (!testFS.is_open()) {
		...
		if (!SetParam(argv[1], "INPUTS", "HAB_FN", "Filename. Habitat input grid file name without extension.") || 
		    ...
		    !SetParam(argv[1], "INPUTS", "FAC_FN", "Filename. Factor input grid file name without extension. Optional, default = not used.") || 
		    !SetParam(argv[1], "OUTPUTS", "NHA_FN", "Filename. NHA output filename without extension.") || 
	...
	testFS.close();
	int returnValue = ContextCBA(argv[1]);
