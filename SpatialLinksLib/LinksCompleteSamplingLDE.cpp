/*
LinksCompleteSamplingLDE.cpp - experimental complete sampling Spatial Links implementaion using new custom heap and linear decay 
...
//Notes
// This Spatial Links implementaion contains requested alterations for a specific purpose. 
// 
//TODO 
...
int LinksCompleteSamplingLDE(std::string paramFN, LinksParams& p) {
	...
	{
		result = LinksCompleteSamplingLDE_ST(p);
	}
	return result;
}
	...
	for (int focalRow = 0; focalRow < p.nRows; focalRow++) {
		...
		for (int focalCol = 0; focalCol < p.nCols; focalCol++) {
			...
			while (true) {
				...
				//Output Context
				//multiply by linear rather than exponential cost
				//if (p.doContext)
					//cxtData[windowCenter][focalCol] += float(exp(-(1.0 / p.d_a) * cCell->cost) * double(habData[cCell->y][cCell->x + xOffset]));
				
				cxtData[windowCenter][focalCol] += float((1.0 - (cCell->cost / p.maxED)) * double(habData[cCell->y][cCell->x + xOffset]));
				...
				for (n = 0; n < 8; n++) {
					...
					//Check cost for no data
					if (cstData[ny][nx + xOffset] == p.noData) continue;

					//Check hab for no data
					if (habData[ny][nx + xOffset] == p.noData) continue;
					...
					if (nAccumCost > p.maxED) continue;
